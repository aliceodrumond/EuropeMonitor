...
<html lang="en">
  ...
  <body>
    ...
    <script>
      ...
      function renderSpeakers(root){ const panel=document.createElement("article"); panel.className="panel wide"; panel.innerHTML='<div class="head"><div><p class="kicker">Communication</p><h2>ECB Speakers</h2></div></div><div class="table-wrap"><table><thead><tr><th>Date</th><th>Member</th><th>Position</th><th>Country</th><th>Headline</th><th>Policy Comments</th><th>Bias</th></tr></thead><tbody>'+speakers.map(s=>'<tr><td>'+s.date+'</td><td>'+s.member+'</td><td>'+s.position+'</td><td>'+s.country+'</td><td>'+(s.source_url?'<a target="_blank" rel="noreferrer" href="'+s.source_url+'">'+(s.headline||"Article")+'</a>':(s.headline||""))+'</td><td>'+s.policy_comments+'</td><td><span class="bias '+s.bias+'">'+s.bias+'</span></td></tr>').join("")+'</tbody></table></div>'; root.appendChild(panel); }
    </script>
  </body>
</html>`;
