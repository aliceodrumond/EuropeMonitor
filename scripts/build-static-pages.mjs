...
<html lang="en">
  ...
  <body>
    ...
    <script>
      ...
      function draw(root, series, chart){ const W=920,H=470,m={t:30,r:54,b:42,l:50}, iw=W-m.l-m.r, ih=H-m.t-m.b, pts=series.flatMap(s=>s.points); if(!pts.length){root.textContent="Waiting for data";return;} const tx=domain(pts.map(p=>p.time)), left=series.filter(s=>s.axis!=="right").flatMap(s=>s.points.map(p=>p.value)), right=series.filter(s=>s.axis==="right").flatMap(s=>s.points.map(p=>p.value)), ly=domain(left.length?left:right), ry=right.length?domain(right):ly; const sx=t=>m.l+(t-tx.min)/(tx.max-tx.min)*iw, sy=(v,a)=>{const d=a==="right"?ry:ly; return m.t+(1-(v-d.min)/(d.max-d.min))*ih}; let svg='<svg viewBox="0 0 '+W+' '+H+'" role="img" aria-label="'+chart.title+'">'; ticks(ly,5).forEach(t=>{const y=sy(t,"left"); svg+='<line class="grid-line" x1="'+m.l+'" x2="'+(W-m.r)+'" y1="'+y+'" y2="'+y+'"/><text class="tick" text-anchor="end" x="'+(m.l-10)+'" y="'+(y+4)+'">'+fmt(t)+'</text>';}); ticks(tx,7).forEach(t=>{const x=sx(t); svg+='<line class="grid-line" x1="'+x+'" x2="'+x+'" y1="'+m.t+'" y2="'+(H-m.b)+'"/><text class="tick" text-anchor="middle" x="'+x+'" y="'+(H-m.b+24)+'">'+new Date(t).getFullYear().toString().slice(2)+'</text>';}); svg+='<line class="axis-line" x1="'+m.l+'" x2="'+m.l+'" y1="'+m.t+'" y2="'+(H-m.b)+'"/><line class="axis-line" x1="'+m.l+'" x2="'+(W-m.r)+'" y1="'+(H-m.b)+'" y2="'+(H-m.b)+'"/><text class="tick" x="'+m.l+'" y="20">'+chart.yLeft+'</text>'; if(chart.yRight) svg+='<text class="tick" text-anchor="end" x="'+(W-m.r)+'" y="20">'+chart.yRight+'</text>'; series.forEach(s=>{ svg+='<path class="line" stroke="'+s.color+'" d="'+s.points.map((p,i)=>(i?'L':'M')+sx(p.time).toFixed(2)+','+sy(p.value,s.axis).toFixed(2)).join(' ')+'"/>'; }); root.innerHTML=svg+'</svg><div class="chart-tooltip" hidden></div>'; const tip=root.querySelector('.chart-tooltip'), svgEl=root.querySelector('svg'); svgEl.addEventListener('mouseleave',()=>{tip.hidden=true}); svgEl.addEventListener('mousemove',ev=>{ const rect=svgEl.getBoundingClientRect(), x=(ev.clientX-rect.left)/rect.width*W, y=(ev.clientY-rect.top)/rect.height*H; if(x<m.l||x>W-m.r||y<m.t||y>H-m.b){tip.hidden=true;return;} const target=tx.min+(x-m.l)/iw*(tx.max-tx.min); const nearest=series.map(s=>{ const p=s.points.reduce((a,b)=>Math.abs(b.time-target)<Math.abs(a.time-target)?b:a); return {s,p,x:sx(p.time),y:sy(p.value,s.axis)}; }); const anchor=nearest.reduce((a,b)=>Math.abs(b.p.time-target)<Math.abs(a.p.time-target)?b:a); tip.innerHTML='<div class="tooltip-date">'+monthLabel(anchor.p.date)+'</div>'+nearest.map(n=>'<div class="tooltip-row"><span class="swatch" style="background:'+n.s.color+'"></span><span>'+n.s.name+'</span><strong>'+fmt(n.p.value)+'</strong></div>').join(''); tip.style.left=(anchor.x/W*100)+'%'; tip.style.top=(Math.min(...nearest.map(n=>n.y))/H*100)+'%'; tip.hidden=false; }); }
      ...
      function monthLabel(date){ return new Intl.DateTimeFormat("en-US",{month:"2-digit",year:"numeric"}).format(new Date(date+"T00:00:00")); }
