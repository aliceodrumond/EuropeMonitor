...
<html lang="en">
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .hicp-summary table { min-width:720px; font-size:.88rem; }
      .hicp-summary th,.hicp-summary td { text-align:right; }
      .hicp-summary th:first-child,.hicp-summary td:first-child { width:38%; text-align:left; }
  ...
  <body>
    ...
    <script>
      ...
      function renderHicpSummary(root){ const panel=document.createElement("article"); panel.className="panel wide hicp-summary"; const stateKey="hicp_summary", st=state.get(stateKey)||{seasonal:"ecb"}; state.set(stateKey,st); const defs=[["HICP Headline","hicp_headline_yoy_nsa","hicp_headline_qoq_saar","hicp_headline_mom_saar"],["HICP Core ex-Energy, Food, Alcohol and Tobacco","hicp_core_yoy_nsa","hicp_core_qoq_saar","hicp_core_mom_saar"],["HICP Non-Energy Industrial Goods","hicp_goods_yoy_nsa","hicp_goods_qoq_saar","hicp_goods_mom_saar"],["HICP Services","hicp_services_yoy_nsa","hicp_services_qoq_saar","hicp_services_mom_saar"]]; const controls='<div class="seasonal">'+["ecb","legacy"].map(k=>'<button class="seasonal-button'+(st.seasonal===k?' active':'')+'" data-seasonal="'+k+'">SA - '+(k==="ecb"?"ECB":"Legacy")+'</button>').join("")+'</div>'; const body=defs.map(d=>{ const q=st.seasonal==="legacy"?d[2]+"_legacy":d[2], m=st.seasonal==="legacy"?d[3]+"_legacy":d[3], y=latest(d[1]), qo=latest(q), mo=latest(m), date=[y,qo,mo].filter(Boolean).sort((a,b)=>b.time-a.time)[0]?.date||""; return '<tr><td>'+d[0]+'</td><td>'+fmtCell(y?.value)+'</td><td>'+fmtCell(qo?.value)+'</td><td>'+fmtCell(mo?.value)+'</td><td>'+(date?dateLabel(date):"")+'</td></tr>'; }).join(""); panel.innerHTML='<div class="head"><div><p class="kicker">Inflation</p><h2>HICP Summary</h2></div>'+controls+'</div><div class="hicp-summary-wrap"><table><thead><tr><th>Breakdown</th><th>% YoY NSA</th><th>% QoQ SAAR</th><th>% MoM SAAR</th><th>Latest</th></tr></thead><tbody>'+body+'</tbody></table></div>'; panel.querySelectorAll("[data-seasonal]").forEach(b=>b.onclick=()=>{st.seasonal=b.dataset.seasonal;render();}); root.appendChild(panel); }
