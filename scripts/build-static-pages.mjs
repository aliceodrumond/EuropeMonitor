...
<html lang="en">
  ...
  <body>
    ...
    <script>
      ...
      function renderTabData(root,tab){ const ids=new Set(charts.filter(c=>c.tab===tab).map(c=>c.id)), rows=allRows.filter(r=>ids.has(r.chart_id)); let html='<span>Last data updated: pending</span><strong>Waiting for data</strong>'; if(rows.length){ const max=Math.max(...rows.map(r=>r.time)), latest=rows.filter(r=>r.time===max), names=[...new Set(latest.map(r=>(charts.find(c=>c.id===r.chart_id)||{}).title||r.series_name))].slice(0,3); html='<span>Last data updated: '+fullDate(new Date(max).toISOString().slice(0,10))+'</span><strong>'+names.join(", ")+'</strong>'; } const panel=document.createElement("section"); panel.className="tab-data"; panel.innerHTML=html; root.appendChild(panel); }
