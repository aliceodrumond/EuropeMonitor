  ...
  <head>
    ...
    <style>
      ...
      * { box-sizing: border-box; }
      ...
      .tab,.legend-button,.window-button,.seasonal-button { border:1px solid var(--line); border-radius:999px; background:#fff; color:var(--muted); min-height:34px; padding:0 12px; font-weight:650; }
      ...
      .window,.seasonal { display:flex; flex-wrap:wrap; justify-content:flex-end; gap:6px; }
      .window-button.active,.seasonal-button.active { border-color:var(--teal); color:var(--teal); background:rgba(17,103,95,.1); }
      ...
      @media (max-width:940px){ .shell{width:min(100% - 20px,1480px);padding-top:16px}.topbar,.head{flex-direction:column}.brand{text-align:left}.status,.controls,.window,.seasonal{align-items:flex-start;justify-content:flex-start}.tabs{gap:20px;justify-content:flex-start}.grid{grid-template-columns:1fr}.panel{padding:14px} svg{min-height:300px} table{min-width:900px;font-size:.84rem} }
    </style>
  </head>
  <body>
    <main class="shell">
    ...
    <script>
      ...
      function render(){ renderTabs(); document.getElementById("status").innerHTML = '<span class="pill">Updated: '+(metadata.last_updated||"pending")+'</span><span class="pill">'+new Set(allRows.map(r=>r.series_id)).size+' series</span>'; document.getElementById("footer").textContent = 'Data mode: '+(metadata.data_mode||"source linked")+'. Data contract: CSVs in public/data.'; const el=document.getElementById("content"); el.innerHTML=""; if(activeTab==="speakers") renderSpeakers(el); else { renderTabData(el,activeTab); if(activeTab==="inflation") renderHicpSummary(el); charts.filter(c=>c.tab===activeTab).forEach(c=>renderChart(el,c)); } }
      ...
      function chartState(chart){ if(!state.has(chart.id)) state.set(chart.id,{window:chart.defaultWindow||"all",hidden:new Set(defaultHidden(chart)),seasonal:"ecb"}); return state.get(chart.id); }
      function styleForSeries(id, fallback){ const m=id.replace("_legacy",""); if(m.endsWith("_yoy_nsa")) return {color:"#111111"}; if(m.endsWith("_qoq_saar")) return {color:"#a83f39"}; if(m.endsWith("_hoh_saar")) return {color:"#11675f"}; if(m.endsWith("_mom_saar")) return {color:"#65b88f",dashArray:"6 5"}; return {color:fallback}; }
      ...
      function fmtCell(v){ return Number.isFinite(v) ? fmt(v) : ""; }
      function fmtChange(v){ return Number.isFinite(v) ? (v>0?"+":"")+fmt(v) : ""; }
